...
- 모든 사용자 문구는 `src/editor/i18n.ts`를 통해 지원 언어 전체(한국어, 영어, 중국어 간체·번체,
  일본어, 스페인어, 프랑스어, 독일어, 브라질 포르투갈어, 러시아어)에 대응한다. 새 문구를
  추가하거나 바꿀 때 **모든 언어의 실제 번역을 함께 추가**하고 한국어 문구를 방치하지 않는다.
- 언어 선택지는 언제나 해당 언어의 자칭(English, 日本語, 简体中文 등)으로 표시한다.
