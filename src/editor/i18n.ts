...
const rows: Record<string, readonly string[]> = {
  ...
  "PNG를 더 작은 WebP로 변환": ["Convert PNG to a smaller WebP", "将 PNG 转换为更小的 WebP", "將 PNG 轉換為更小的 WebP", "PNGをより小さいWebPに変換", "Convertir PNG a un WebP más pequeño", "Convertir le PNG en WebP plus petit", "PNG in ein kleineres WebP umwandeln", "Converter PNG em um WebP menor", "Преобразовать PNG в WebP меньшего размера"],
  "현재 PNG를 WebP로 압축합니다. 변환 파일이 원본보다 작을 때만 프로젝트에 적용합니다.": ["Compresses the current PNG as WebP and applies it only when the result is smaller.", "将当前 PNG 压缩为 WebP，仅在结果更小时应用。", "將目前 PNG 壓縮為 WebP，僅在結果較小時套用。", "現在のPNGをWebPに圧縮し、元より小さい場合だけ適用します。", "Comprime el PNG actual como WebP y solo lo aplica si es más pequeño.", "Compresse le PNG actuel en WebP et ne l’applique que s’il est plus petit.", "Komprimiert das aktuelle PNG als WebP und übernimmt es nur, wenn es kleiner ist.", "Comprime o PNG atual como WebP e só aplica se ficar menor.", "Сжимает текущий PNG в WebP и применяет результат, только если он меньше."],
  "투명 배경은 유지됩니다 · WebP가 같거나 크면 PNG를 유지합니다": ["Transparency is preserved · PNG is kept if WebP is the same size or larger", "保留透明背景 · 若 WebP 大小相同或更大则保留 PNG", "保留透明背景 · 若 WebP 大小相同或更大則保留 PNG", "透過背景は維持 · WebPが同じか大きければPNGを維持", "Se conserva la transparencia · Se mantiene PNG si WebP pesa igual o más", "La transparence est préservée · Le PNG est gardé si WebP est aussi grand ou plus grand", "Transparenz bleibt erhalten · PNG bleibt, wenn WebP gleich groß oder größer ist", "A transparência é preservada · O PNG é mantido se o WebP for igual ou maior", "Прозрачность сохраняется · PNG остаётся, если WebP такого же или большего размера"],
  "WebP가 더 작지 않아 PNG를 유지했습니다.": ["WebP was not smaller, so the PNG was kept.", "WebP 并未更小，因此保留了 PNG。", "WebP 並未更小，因此保留 PNG。", "WebPの方が小さくないためPNGを維持しました。", "WebP no era más pequeño; se conservó el PNG.", "WebP n’était pas plus petit ; le PNG a été conservé.", "WebP war nicht kleiner; PNG wurde beibehalten.", "O WebP não ficou menor; o PNG foi mantido.", "WebP не оказался меньше, поэтому PNG сохранён."],
  "WebP 변환 시 화질이 달라져 PNG를 유지했습니다.": ["The PNG was kept because WebP changed the image quality.", "WebP 转换改变了画质，因此保留了 PNG。", "WebP 轉換改變了畫質，因此保留 PNG。", "WebP変換で画質が変わったためPNGを維持しました。", "Se conservó el PNG porque WebP cambió la calidad de imagen.", "Le PNG a été conservé car WebP a modifié la qualité de l’image.", "PNG wurde beibehalten, weil WebP die Bildqualität verändert hat.", "O PNG foi mantido porque o WebP alterou a qualidade da imagem.", "PNG сохранён, поскольку WebP изменил качество изображения."],
