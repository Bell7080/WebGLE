...
- `deform`은 움직임과 Mesh 변형을 조합한 다음 네 값 중 하나다.
  네 값 모두 가중치를 기준으로 적용한다. 완전 고정이 다른 관절보다 큰 주 영향인 정점은
  부모·이웃 영향을 끊고 아예 멈추며, 같은 비율로 겹치는 경계는 섞여 부드럽게 이어진다.
...
`resolution`은 `"low"`(24칸) · `"normal"`(48) · `"high"`(72) · `"ultra"`(108) 중 하나다.
**이 이름들의 칸 수는 바뀌지 않는다.** 파일에는 이름만 적히고 격자는 열 때 다시 만들어지므로,
같은 이름의 칸 수를 바꾸면 예전 파일의 정점 수가 어긋나 칠해 둔 영향 영역이 통째로 날아간다.
더 촘촘한 것이 필요하면 이름을 새로 늘린다(`ultra`가 그렇게 생겼다).
편집기 화면에 보이는 이름표(`최소` · `낮음` · `보통` · `높음`)는 이 값과 별개로 관리된다.
