...
`facing`은 그림 속 캐릭터가 바라보는 쪽이다. 없으면 `"right"`로 본다.
프리셋은 전부 오른쪽을 보는 캐릭터 기준으로 만들어져 있어서, 왼쪽을 보는 그림에 그대로 얹으면
등 뒤로 주먹을 뻗는다. `"left"`면 재생할 때 가로 이동(`x`)과 회전의 부호가 뒤집힌다.
그림을 좌우로 뒤집는 것이 아니라 **동작의 방향만** 돌리는 것이며, 런타임이 알아서 처리하므로
게임 쪽에서 따로 챙길 것은 없다.
